...
def main(args):
    ...
    image_root_path = root_dir / "images"
    ...
    images = {
        image_path.name.split(".")[0]: PIL.Image.open(image_path)
        for image_path in image_root_path.glob("*.[jpJP][npNP]*[gG$]")
    }

    ip2p = InstructPix2PixSDXL(device=device, ip2p_use_full_precision=False)

    text_embeddings = ip2p.pipe.encode_prompt(
        text_prompt,
        device=device,
    ...
    for name, image in images.items():
        ...
        # image_latent = ip2p.imgs_to_latent(image_tensor)

        original_image = image

        # in the Gaussian pipelines

        image_tensor = ip2p.pipe.image_processor.preprocess(image)
        original_image_tensor = ip2p.pipe.image_processor.preprocess(original_image).to(device)
        image_cond = ip2p.prepare_image_latents(original_image_tensor)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser("Prepare stylized targets by using InstructPix2Pix")
    parser.add_argument(
        "--root_dir",
        type=str,
        default="/home/lucky/dataset/gauss-dataset/fangzhou-small",
    ...
    parser.add_argument("--guidance_scale", type=float, default=7.5, help="Weight of the text prompt")
    parser.add_argument(
