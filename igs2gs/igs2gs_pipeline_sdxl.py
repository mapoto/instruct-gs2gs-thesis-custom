...
from test_script.ip2pxl_custom import InstructPix2PixSDXL
...
class InstructGS2GSPipelineConfig(VanillaPipelineConfig):
    ...
    """prompt for InstructPix2PixSDXL"""
    guidance_scale: float = 12.5
    """(text) guidance scale for InstructPix2PixSDXL"""
    image_guidance_scale: float = 1.5
    """image guidance scale for InstructPix2PixSDXL"""
    gs_steps: int = 2500
    """how many GS steps between dataset updates"""
    diffusion_steps: int = 20
    """Number of diffusion steps to take for InstructPix2PixSDXL"""
    ...
    """Second device to place InstructPix2PixSDXL on. If None, will use the same device as the pipeline"""
    ip2p_use_full_precision: bool = False
    """Whether to use full precision for InstructPix2PixSDXL"""
...
class InstructGS2GSPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        self.ip2p = InstructPix2PixSDXL(self.ip2p_device, ip2p_use_full_precision=self.config.ip2p_use_full_precision)

        # load base text embedding using classifier free guidance
        self.text_embedding = self.ip2p.pipe.encode_prompt(
            self.config.prompt,
