...
def process_images_in_batches(clip_model, images, batch_size):
    image_features_list = []
    for i in range(0, len(images), batch_size):
        batch_images = images[i : i + batch_size]
        batch_tensor = torch.cat(batch_images).to("cuda")
        with torch.no_grad():
...
def main(folder_path, model_name="ViT-L/14", top_n=3, batch_size=4):
    ...
    image_features = process_images_in_batches(clip_model, images, batch_size)
