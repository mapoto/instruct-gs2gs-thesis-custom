...
def compute_rmse_mask(
    ...
    for i in range(len(masked_batch)):
        ...
        results.append(mean_abs_diff.item())
        torch.cuda.empty_cache()

    return np.array(results).mean()


def compute_rmse_per_iter(data_dir, adjacency_batch, lpips_model, save_folder, results_name="Ephra"):
...
if __name__ == "__main__":
    ...
    for folder in root.iterdir():
        if folder.is_dir():
            iter = folder.stem
            # if (int(iter) % 200 == 0) or (int(iter) % 2500 == 1):
    ...
    for folder in folders:
        ...
        filtered = compute_rmse_per_iter(
            data_dir=folder,
        ...
        # save plane rmse
        np.savetxt(save_folder / f"{iter}_ciede2000.csv", filtered, delimiter=",")
