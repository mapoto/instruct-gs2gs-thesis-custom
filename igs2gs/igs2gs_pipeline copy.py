...
class InstructGS2GSPipelineConfig(VanillaPipelineConfig):
    ...
    incremental_guidance_scale: float = 1.0
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if self.makeSquentialEdits:
            ...
            while clip_ok:
                ...
                edits_batch = [edits_list[i : i + 4] for i in range(0, edits_length, 4)]

                for camera_idx in range(0, len(self.datamanager.train_data), self.config.batch_size):
                    batch_idx = list(range(camera_idx, camera_idx + self.config.batch_size))
                    print(f"Editing image {",".join(batch_idx)}")
                    ...
                    rendered_image.save(Path(self.img_outpath / str(step) / f"{str(camera_idx)}_render.png"))

                    # save original image
                    original_image = self.to_image(original_image.squeeze(0).permute(1, 2, 0))
                    original_image.save(Path(self.img_outpath / str(step) / f"{str(camera_idx)}_original.png"))
                    ...
                    text_embedding = torch.cat([self.text_embedding] * len(batch), dim=0)
                    ...
                    for camera_index in batch:

                        tensor_index = batch.index(camera_index)
                        edited_image = edited_batch[tensor_index]
                        edited_images[str(camera_index)] = (edited_image[:3, :, :].unsqueeze(0) * 255).byte()

                        # as H x W x C tensor
                        edited_image = edited_image.squeeze().permute(1, 2, 0)

                        # update dataset with edited image
                        ...
                        self.curr_edit_num += 1

                clip_average, clip_similarity = self.do_csmv(list(edited_images.values()))

                reedit_candidates = cm.find_least_similar(clip_similarity, list(edited_images.keys()), threshold=0.85)
