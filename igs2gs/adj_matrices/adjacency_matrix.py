...
image_root_path = Path("/home/lucky/dataset/metashape_aligned/Ephra_grn/clean/")
camera_names = [image_path.name for image_path in image_root_path.glob("*.[jpJP][npNP]*[gG$]")]


# Parse camera names
def parse_camera_name(name):
    match = re.match(r"(\d+)-([1-3BC])-([1-6])-([1-2])-\d+-\d+\.png", name)
    if match:
    ...
    for j, cam2 in enumerate(cameras):
        if i != j:
            ...
            if (
                ...
                # Ensure columns "1" and "C" are not considered adjacent
