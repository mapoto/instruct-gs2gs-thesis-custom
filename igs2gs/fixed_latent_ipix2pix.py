...
class FixedLatentInstructPix2Pix(
    DiffusionPipeline,
    ...
    image_cond_latents: Optional[torch.FloatTensor]
    ...
    def __call__(
        ...
        image_cond_latents: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        image_latents = image_cond_latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_img = latent_model_input[0].squeeze().permute(1, 2, 0).to(image.dtype).detach().cpu().numpy()
                latent_img = (latent_img * 255).astype(np.uint8)

                # Convert to PIL Image
                latent_img = PIL.Image.fromarray(latent_img)

                # Save the image
                latent_img.save(latents_path / f"latent_{i}.png")

                # concat latents, image_latents in the channel dimension
                scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                # print("scaled_latent_model_input", scaled_latent_model_input.shape)
